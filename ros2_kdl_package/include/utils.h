                          ...
                          Vector6d &edot)
{
    ...
    //computeOrientationVelocityError(toEigen(_Vd.rot), toEigen(_Ve.rot), R_d, R_e);
}
...
inline Eigen::Matrix<double,3,3> computeT(const Eigen::Matrix<double,3,1> &euler){
...
inline Eigen::Matrix<double,3,3> computeTdot(const Eigen::Matrix<double,3,1> &euler){
    double phi = euler(0,0);
    double theta = euler(1,0);
