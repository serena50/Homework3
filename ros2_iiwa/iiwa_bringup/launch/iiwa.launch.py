...
def generate_launch_description():
    ...
    DeclareLaunchArgument(
        'use_vision',
        default_value='false',
        description='Enable camera on the end-effector',
    )
)
    ...
    use_vision = LaunchConfiguration('use_vision')  # Added for the 1b
    ...
    robot_description_content = Command(
        [
            ...
            use_sim,
            ' ',
            'use_vision:=',  # Added to accomplish 1b)
            use_vision,      # And this one for the 1b)
            ' ',
            'use_fake_hardware:=',
            use_fake_hardware,
            ' ',
    ...
    iiwa_simulation_world = PathJoinSubstitution(
        [FindPackageShare(description_package),
            'gazebo/worlds', 'empty_spherical.world']
    )
